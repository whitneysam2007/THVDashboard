...
function mapDbDonor(row: Record<string, unknown>): Donor {
  return {
    ...
    donationYearTotals: Object.fromEntries(Object.entries((row as any).donationYearTotalsCents ?? {}).map(([year, amount]) => [year, Number(amount ?? 0) / 100])),
    currentYearLatestDonation: (row as any).currentYearLatestDonation
      ? { amount: Number((row as any).currentYearLatestDonation.amountCents ?? 0) / 100, date: (row as any).currentYearLatestDonation.date }
      : undefined,
